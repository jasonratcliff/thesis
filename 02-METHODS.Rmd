...
For project reproducilibilty, source code and data were maintained
...
spp_tree_hypotheses <-
  system.file("extdata/BEAST", package = "ThesisPackage") %>%
  list.files(pattern = "spp_hypotheses.csv", full.names = TRUE) %>%
  readr::read_csv(file = ., col_types = 'ccccc') %>%
  setNames(c("Sample", paste("Hypothesis", 1:4))) %>%
  arrange(`Hypothesis 1`, Sample)

if (knitr_chunk == "latex") {
  spp_tree_hypotheses <- spp_tree_hypotheses %>%
    mutate(
      Sample = gsub("_", "\\\\_", .data$Sample)
    ) %>%
    mutate_at(
      vars(matches("^Hypothesis")),
      ~ paste0("\\textit{", .x, "}") %>%
        gsub(" ssp ", "} \n \\\\hspace{0.8em} ssp. \\\\textit{", x = ., ) %>%
        kableExtra::linebreak(x = .) %>%  # Fix latex linebreak cell alignment
        gsub("makecell\\[[rc]\\]", "makecell\\[l\\]", x = .)
    )
  }

spp_tree_hypotheses %>%
  kable(x = ., format = knitr_chunk, align = "l", escape = FALSE,
        caption = "Species tree hypotheses taxa assignments by sample ID."
        ) %>%
