...
```{r inlineVariables, echo=FALSE}
# Calculate SEINet records with non-missing geographic coordinates.
seinet_geo <- dplyr::filter(seinet_coords, !is.na(Latitude), !is.na(Longitude))
...
knitr_section(
  knitr_title = "Herbarium Specimens",
...
packages [@R-stringr] to subsitute the species name for identification
...
Raster images for satellite maps and digital elevation model data were obtatined
...
Additional specimen occurence data were obtained from the
...
`r nrow(seinet_geo)` with specimen occurrence decimal degree coordinates.
...
effect the accuracy of phylogenetic inference. Simulation studies by
...
`r knitr_table_dna(knitr_chunk)`). For the narrowly distributed Wyoming state
endemics *P. dornii* and *P. condensata*, four individuals were sampled from
each taxa. For Wyoming state endemics *P. eburniflora* and *P. saximontana*
ssp. *saximontana*, five individuals from each taxa were included. Four
individuals of *P. saximontana* ssp. *dentata* were sampled broadly from
Montana. Five specimens identified as *P. didymocarpa* ssp. *lanata* in Wyoming
and three identified as *P. didymocarpa* ssp. *lyrata* in Idaho were included.
These two *P. didymocarpa* subspecies have narrow distributions and are thought
to be state endemics of Wyoming and Idaho, respectively. Five individuals were
sampled from *P. didymocarpa* ssp. *didymocarpa* in Wyoming and Montana.

\paragraph{} For the regional endemics *P. brassicoides* and *P. integrifolia*,
six and five individuals were sampled, respectively. For *P. vitulifera*, three
individuals from Colorado and four putative specimens with annotations as
*P. vitulifera* from Carbon County, Wyoming were included. The Carbon County
population is of interest due to the questionable status of this species’
distribution in the state of Wyoming [@HeidelOKane2012]. Finally, twelve
individuals of the widespread *P. acutifolia* from Utah, Colorado, and Wyoming
were included. Sequence data were obtained from `r length(fasta_list_raw$rITS)`
specimens for the rITS locus, `r length(fasta_list_raw$rps)` for the *rps*
intron, and `r length(fasta_list_raw$ycf1)` for *ycf1*. Analyses of concatenated multi-locus
or partitioned data were limited to specimens for which sequencing of all three
loci was completed.
...
dna_collections <-
  list.files(system.file("extdata/Methods", package = "ThesisPackage"),
...
dna_footnote <-
  paste0("Requested herbarium specimens used for extraction of ",
         ...
         "indicate specimens with sequence data obtained by Dr. O'Kane.")
```

```{r TableDnaSpecimensLatex}
# LaTeX specimen table part 1
if (knitr_chunk == "latex") {
  dna_collections %>% dplyr::slice(1:39) %>%
    kable(x =., format = knitr_chunk, escape = FALSE,
    ...
    footnote(general = dna_footnote, general_title = "",
...
```{r TableDnaSpecimensLatexCont, tab.cap = NULL}

# LaTeX specimen table part 2
if (knitr_chunk == "latex") {
  dna_collections %>% dplyr::slice(40:68) %>%
    kable(x = ., format = knitr_chunk, escape = FALSE,
...
\setcounterref{newcount}{tab:TableDnaSpecimensLatex}
...
```{r sectionExtractions}

knitr_section(
  knitr_title = "DNA Extraction",
  knitr_type = "section", chunk_type = knitr_chunk
)
...
Idaho, and Montana with Dr. S. L. O’Kane Jr. Dried leaf tissues were stored at
-20 $^{\circ}$C prior to extraction. DNA was extracted from only a single
...
@Rethwisch2014 in addition to data previously obtained by Dr. O'Kane were
...
4.5 mm zinc plate BBs and two 2.3 mm stainless steel beads. Samples were
...
# Table: PCR Primers
list.files(system.file("extdata/Methods", package = "ThesisPackage"),
             pattern = "DNA-primers", full.names = TRUE) %>%
  readr::read_csv(file = ., col_names = TRUE, col_types = "ccc") %>%
  kable(x = ., format = knitr_chunk, align = "l", escape = FALSE,
...
Gels were run for 30 to 60 minutes at 130 volts, then washed with de-ionized
...
\paragraph{} PCR products were sent to the Iowa State University DNA facility
...
multi-FASTA sequences (rITS: n=`r length(fasta_list_raw$rITS)`,
*rps*: n=`r length(fasta_list_raw$rps)`,
*ycf1*: n=`r length(fasta_list_raw$ycf1)`) and the subset of
...
performed separarately for each locus with the full multi-FASTA files
...
\paragraph{} Nexus files output by 2Matrix were were partitioned to
...
generations. Two parallel analyses were run, each with 4 chains and a temp
parameter of 0.04. A third of the trees were discarded as the burn-in fraction
(0.33), and the stop value was set to 0.001.
...
*.xml* files for were configured using BEAUti v2.6.3 [@Drummond2012].
...
treated as outgroup taxa in the MrBayes inferences were exluded from BEAST
...
a coalesescent constant population tree prior was set with a 1/X population
...
[@Bouckaert2019] with 10,000 resampled states.
...
incomplete lineage sorting was use to estimate a species tree
...
Site and clock model paramaters were retained from the partitioned
multi-locus analysis and a yule model was set for the species tree prior.
The multi species coalescent model was set as a linear population function with
...
\paragraph{} To plot phylogenetic tree results, the `treeio` R pacakge
[@R-treeio2020] was used to read in the MrBayes consensus and BEAST maximum
clade credibility trees. Inferred phylogenies were plotted with the `ggplot2`
extension `ggtree` [@R-ggtree; @R-ggtree2020], with annotations for duplicate
sequences from MrBayes included using `dplyr` and `purrr`. Sequence alignment
matrices were visualized using `msa` [@R-msa], an R package that provides
wrapper functions to print sequences alignments with the $LaTeX$ package
$TeX$shade [@TeXshade].
Additional $LaTeX$ packages utilized for document typesetting include
`titletoc` [@titletoc] for table of contents, `titlesec` [@titlesec] for
section titles, and `titleps` [@titleps] for page styling.

\paragraph{} For project reproducilibilty, source code and data were maintained
as an R package [@R-packages] utilizing the `usethis` [@R-usethis]
and `devtools` [@R-devtools] packages.
A project library with specific package versioning was maintained using
the R package `renv` [@R-renv].
A PDF document and figures from built from source using `bookdown`
[@R-bookdown; @R-bookdown2016], `rmarkdown` [@R-rmarkdown; @R-rmarkdown2018],
and `knitr` [@R-knitr; @R-knitr2015] with a `tinytex` [@R-tinytex] $LaTeX$
distribution for typesetting. The GNU `make` utility v3.81 [@GNUmake] was used
to automate figure generation and document compilation.
References were maintained using Zotero v5.0.93 [@Zotero] with the
Better BibTeX v5.2.86 [@BetterBibTeX] add-on.
The R package source code is available from
https://github.com/jasonratcliff/ThesisPackage
and the manuscript source code is available from
https://github.com/jasonratcliff/Thesis as GitHub repositories.
