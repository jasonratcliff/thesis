...
test_that("Specimen layer works", {
  ...
  aes_color <-
    layer_specimens(
      specimen_tbl = spp_co_front_range, shape_aes = FALSE,
  ...
  expect_warning(
    object = ggplot2::ggplot_build(specimen_ggplots[[1]]),
...
test_that("Border layer works", {
  border_layer <- ggplot() +
    layer_borders(
      spl_extent = spl_bbox(Thesis::spp_co_front_range),
      sf_crs = 3857
    )
  expect_s3_class(
    object = border_layer$layers[[1]]$geom,
...
test_that("Build map works", {
  build_base <-
    build_map(
      specimen_tbl = spp_co_front_range, id_column = "prior_id",
