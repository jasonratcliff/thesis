...
knitr_section(
  knitr_title = "Morphology",
  knitr_type = "section", chunk_type = knit_variables$kable_format
)

```

\paragraph{} Two key morphological features for distinguishing *Physaria*
sensu stricto include the membranous parition separating fruit locules
(i.e., replum; Figure \@ref(fig:traitReplumShape)) and the number of ovules
per locule (Figure \@ref(fig:traitOvules)). Replum shape differs in species
with a constricted replum noted as a linear appearance, from those with a
wider replum varying from oblong or elliptic to oblanceolate or obovate.
Species in the former group include the widespread *P. acutifolia*,
*P. vitulifera*, and *P. brassicoides*, while the latter group includes
*P. didymocarpa* and *P. saximontana* subspecies, *P. eburniflora*,
*P. integrifolia*, *P. dornii*, and *P. condensata*. The distininction of
...
```{r traitReplumShape, fig.cap=caption$traitReplumShape, fig.scap="Distribution of fruit replums"}
...
knitr::include_graphics(
  path = system.file(
    fs::path("figures/DiscussionTraitReplums", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
```{r traitOvules, fig.cap=caption$traitOvules, fig.scap="Distribution of fruit ovules"}
...
knitr::include_graphics(
  path = system.file(
    fs::path("figures/DiscussionTraitOvules", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
\paragraph{} The main charcter used by @Rollins1984 to differentiate
*Physaria saximontana* from *P. didymocarpa* is the lack of a basal sinus in
the mature siliques of the former. This traditional circumsription is followed
...
*P. eburniflora* individuals (Figure \@ref(fig:traitFruitApices)).
...
```{r traitFruitApices, fig.cap=caption$traitFruitApices, fig.scap="Distribution of mature silique sinuses"}
...
knitr::include_graphics(
  path = system.file(
    fs::path("figures/DiscussionTraitApices", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
\paragraph{} @Rollins1939 noted of *Physaria didymocarpa* var. *integrifolia*
Rollins (later, *Physaria integrifolia* (Rollins) @Lichvar1984) appressed
stellate trichomes on the mature siliques and entire basal leaves as
distinguishing characters from *P. d.* subsp. *didymocarpa*.
...
(Figure \@ref(fig:traitFruitTrichomes), Figure \@ref(fig:traitBasalLeafMargins)).
...
```{r traitFruitTrichomes, fig.cap=caption$traitFruitTrichomes, fig.scap="Distribution of silique trichomes"}
...
knitr::include_graphics(
  path = system.file(
    fs::path("figures/DiscussionTraitTrichomes", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
\paragraph{} Of the two *P. saximontana* varieties (var. *saximontana* and
...
(Figure \@ref(fig:traitBasalLeafMargins)). Though common in occurrence, dentate
margins are not supported as a fixed trait in *P. didymocarpa*. In contrast,
*P. acutifolia* and the southwestern endemics *P. integrifolia*, *P. dornii*,
and *P. condensata* generally have entire basal leaves with the former
distinguished from the endemics by replum shape and ovule count.

```{r traitBasalLeafMargins, fig.cap=caption$traitBasalLeafMargins, fig.scap="Distribution of basal leaf margins"}
...
knitr::include_graphics(
  path = system.file(
    fs::path("figures/DiscussionTraitMargins", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
