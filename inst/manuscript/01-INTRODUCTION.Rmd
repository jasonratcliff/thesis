...
carbon <- list()
carbon$specimens <- herbarium_specimens %>%
  dplyr:: select(
    prior_id, Taxon, Date_parsed,
    Collector, Collection_Number,
    State, County, Herbarium
  ) %>%
  dplyr::filter(
    grepl("vitulifera", x = .data$Taxon) &
      grepl("Carbon", x = .data$County)
  ) %>%
  dplyr::mutate(
    # Remove collector initials and punctuation.
    ...
    Annotation = purrr::map2_chr(
      .x = prior_id, .y = Collection_Number,
      .f = function(id, collection) {
        annotation <-
          ifelse(  # italicize entries based on pandoc conversion.
            test = knit_variables$pandoc == "latex",
            yes = paste0("\\textit{", id, "}"),
            no = paste0("*", id, "*")
          )
        annotation <-
          ifelse(  # add collection asterisks.
            test = collection %in%
              c("411", "3695", "63", "54", "19075", "19078", "11363",
                "16699", "19073", "16713", "19510", "6", "16475"),
            yes = paste0(
              annotation,
              ifelse(  # preserve markdown italicization for GitBook.
                test = knit_variables$pandoc == "latex",
                yes = "*",
                no = "\\*"
              )
  ...
  dplyr::select(Annotation, Collector, Collection_Number, Herbarium)
...
carbon$kable <-
  knitr::kable(
    ...
    caption = paste0(caption$TableIntroCarbon, "."),
    caption.short = caption$TableIntroCarbon
  )
...
```{r FigureIntroCarbonWyo, fig.align = "center", fig.cap=caption$FigureIntroCarbon, fig.scap=" \\textit{Physaria} of the Colorado Basin, Front Range, and Medicine Bow Mountains"}
...
caption$FigureIntroCarbon <-
  paste(
    ...
    "White diamonds indicate sampled DNA specimens",
    "and yellow squares denote reviewed voucher specimens of interest.",
