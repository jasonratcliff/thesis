...
*P. acutifolia* (Figure 1.1), spans Montana, Idaho, Wyoming, South Dakota, Colorado, Utah, Nevada, Arizona and New Mexico and forms a cline with plants at the higher elevations being smaller, often with branched, sometimes elongated, caudices, and elongated styles (O’Kane 2010) Infraspecific taxa not recognized
...
```{r FigureIntroPriors, fig.cap=caption$FigureIntroPriors, fig.scap="\\textit{Physaria} sensu stricto herbarium specimen records by prior annotation"}

caption$FigureIntroPriors <-
  paste(
    "Distribution of", physaria(knitr_chunk = knit_variables$kable_format),
    "sensu stricto herbarium specimens by most recent annotations."
  )

knitr::include_graphics(
  system.file(
    fs::path("figures/IntroPriors", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
species from south-central Wyoming, distinguished by fruits with a somewhat
...
described as occurring on rocky slopes in or nearby mountains [@Dorn2001].
...
```{r FigureIntroCarbon, fig.cap=caption$FigureIntroCarbon, fig.scap="Putative \\textit{Physaria vitulifera} specimens with SEINet occurrences of related species"}

caption$FigureIntroCarbon <-
  paste(
    "Distribution of reported Carbon County, Wyoming",
    ifelse(
      knit_variables$pandoc == "latex",
      "\\textit{Physaria vitulifera}.",
      "*Physaria vitulifera*."
    ),
    "SEINet specimen occurrences reveal ranges of neighboring",
    ifelse(
      knit_variables$pandoc == "latex",
      "\\textit{P. floribunda}", "*Physaria floribunda*"
    ),
    "supspecies southwest from Middle Park and",
    ifelse(
      knit_variables$pandoc == "latex",
      "\\textit{P. bellii}", "*Physaria bellii*"
    ), "north of",
    ifelse(
      knit_variables$pandoc == "latex",
      "\\textit{P. vitulifera}", "*Physaria vitulifera*"
    ), "along the eastern slope of the Front Range in Colorado."
  )

knitr::include_graphics(
  system.file(
    fs::path("figures/IntroCarbonWyo", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
