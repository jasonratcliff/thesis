...
Other subspecies of *Physraia didymocarpa*, subsp. *lanata* and subsp.
*didymocarpa*, fruit into July and August with sizes in the range of 7-12 mm.

\vspace*{24pt}

```{r FigResultsPhysariaOvules, fig.cap=caption$FigResultsPhysariaOvules, fig.scap="Counts of maximum ovules per locule"}

caption$FigResultsPhysariaOvules <-
  "Counts of maximum ovules per locule."

knitr::include_graphics(
  path = system.file(
    fs::path("figures/ResultsPhysariaOvules", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
knitr::include_graphics(
  path = system.file(
    fs::path("figures/ResultsPhysariaFruits", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
results$elevation$specimens <- ThesisPackage::herbarium_specimens %>%
  ThesisPackage::filter_reviewed(specimen_tbl = .) %>%
  dplyr::filter(
    !is.na(Elev_raw_max),
...
knitr::include_graphics(
  path = system.file(
    fs::path("figures/ResultsPhysariaElevation", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
...
which the inflorescence exceed the basal rosette reveal three species with
...
by from other `r physaria(knitr_chunk = knit_variables$pandoc)` species by
...
knitr::include_graphics(
 path = system.file(
    fs::path("figures/ResultsPhysariaHabit", ext = knit_variables$graphics),
    package = "Thesis"
  )
)
