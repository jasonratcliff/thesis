...
abbreviate_spp <- function(label_vector, knitr_chunk) {
...
conserved_rITS <-
  ...
  dplyr::mutate(
    Priors = abbreviate_spp(label_vector = .data$Priors, knitr_chunk)
  ) %>%
...
*P. integififolia* (n=3); 6 individuals of *P. acutifolia* from 
...
```{r FigureBayesrITS, fig.cap=caption$FigureBayesrITS}

caption$FigureBayesrITS <-
  "Consensus tree from Baysian inference of the rITS gene tree."

knitr::include_graphics(
  path = paste0("Figs/FigResultsBayes_rITS.", knitr_ext[[knitr_chunk]])
)
...
and western Colorado in addition to three specimens from near Big Horn Canyon.
...
of *P. saximontana* subsp. *dentata*, four specimens of *P. brassicoides*,
...
Fourteen identical sequences (**Genotype 1**) includes specimens previously
...
(n=3). These samples are primarily from populations of the Big Horn Moutains
...
in addition to *P. integrifolia* from Lincoln County, Wyoming; *P. acutifola*
...
caption$TableIdenticalSeqrps <-
  paste(
    physaria(knitr_chunk), "DNA specimens with identical genotypes for the",
    ifelse(knitr_chunk == "html", "*rps*", "\\textit{rps}"), "intron locus."
  )
...
*P. saximontana* subsp. *saxmimontana* (n=3), *P. s* subsp. *dentata* (n=2),
...
```{r FigureBayesrpsIntron, fig.cap=caption$FigureBayesrpsIntron}

caption$FigureBayesrpsIntron <-
  "Consensus tree from Bayesian inference of *rps* intron gene tree."

knitr::include_graphics(
  path = paste0("Figs/FigResultsBayes_rps.", knitr_ext[[knitr_chunk]])
)
...
conserved_ycf1 <-
  ...
  dplyr::mutate(., 
    ...
    Species = abbreviate_spp(label_vector = .data$Species, knitr_chunk)
  ) %>%
...
caption$TableIdenticalSeqycf1 <-
  paste(
    ...
    ifelse(knitr_chunk == "html", "*ycf1*", "\\textit{ycf1}"), "loci.")
...
identified as *P. dornii* and *P. condensata*, and another comprised
...
```{r FigureBayesMultiLocus, fig.cap=caption$FigureBayesMultiLocus}

caption$FigureBayesMultiLocus <-
  "Consensus tree from Bayesian inference of concatenated multi-locus data."

knitr::include_graphics(
  path = paste0("Figs/FigResultsBayes_ML.", knitr_ext[[knitr_chunk]])
)
...
```{r FigureResultsBeastMultiLocus, fig.cap=caption$FigureResultsBeastMultiLocus}

caption$FigureResultsBeastMultiLocus <-
  "BEAST multi-locus partitioned phylogeny."

knitr::include_graphics(
  path = paste0("Figs/FigResultsBeastMultiLocus.", knitr_ext[[knitr_chunk]])
)
...
```{r FigureResultsBeastGeography, fig.cap=caption$FigureResultsBeastGeography}

caption$FigureResultsBeastGeography <-
  "BEAST multi-locus with phylogeography modelling."

knitr::include_graphics(
  path = paste0("Figs/FigResultsBeastGeography.", knitr_ext[[knitr_chunk]])
)
...
```{r FigureResultsBeastSpecies, fig.cap=caption$FigureResultsBeastSpecies}

caption$FigureResultsBeastSpecies <-
  "*BEAST species tree hypotheses."

knitr::include_graphics(
  path = paste0("Figs/FigResultsBeastSpecies.", knitr_ext[[knitr_chunk]])
)
