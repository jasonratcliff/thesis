...
#
# Assign directory with specimen data.
specimen_file <- paste0(here::here(),
...
specimens_raw <-
  ...
  purrr::map(function(excel_sheet) {
    readxl::read_xlsx(path = specimen_file, sheet = excel_sheet,
...
specimens_parsed <- dplyr::bind_cols(specimens_raw %>%
  ...
  plyr::ldply(.data = specimens_split, rbind) %>% tibble::as_tibble(),
  specimens_raw %>% dplyr::select(Taxon:`Chromosome #`))
...
herbarium_specimens <-
  dplyr::bind_cols(dplyr::select(specimens_parsed, excel_sheet:Elev_ft),
    elev_parsed, dplyr::select(specimens_parsed, TRS1:`Chromosome #`))
