...
```{r introductionVariables, echo=FALSE}

introduction_variables <- list()  # Initialize list for assigned variables
...
valves are separated by a constricted replum [@Gray1848; @Watson1888].
Twelve such species have described distributions at least partially in Wyoming:
...
species traditionally recognized in *Physaria* and those of the closely related
...
*L. utahensis* Rydberg, “...great similarity, in some of its forms particularly,
...
need for further work on these taxa and described subspecies.  Lastly, the state
endemic *P. dornii* of southwestern Wyoming has a described distribution
overlapping with the regional endemic *P. integrifolia* and a peripatric
distribution with *P. condensata* [@Lichvar1983].
...
```{r sectionTaxaHistory}

knitr_section(
  knitr_title = "Systematic and Taxonomic History",
...
*P. didymocarpa* var. *didymocarpa* populations near Malad in Oenida County
...
form either soboles or single caudexes, an example of intraspecific variation
in caudex branching.
...
```{r sectionDistribution}

knitr_section(
  knitr_title = "Geographical Distribution and Ecology",
  knitr_type = "section", chunk_type = knit_variables$pandoc
)
...
[@Rollins1939; @Rollins1993]. The most widespread *Physaria* species considered
here, *P. acutifolia* (Figure \@ref(fig:FigureIntroPriors)) forms a cline
spanning Montana, Idaho, Wyoming, South Dakota, Colorado, Utah, Nevada, Arizona
and New Mexico [@OKane2010]. Infraspecific taxa not recognized by @OKane2010
include *P. acutifolia* var. *purpurea* @WelshReveal1977 of Grand County,
Utah and var. *stylosa* @Welsh1986 of northeastern Utah, the latter previously
described as *P. stylosa* by @Rollins1993.

\paragraph{} Another relatively widespread species is *P. didymocarpa*
(Hooker) A. Gray (Figure \@ref(fig:FigureIntroPriors)), which includes three
...
of Lehmi County [@Rollins1993; @OKane2010].

\paragraph{} Three species of *Physaria* sensu-stricto have been described from
southwestern and west-central Wyoming (Figure \@ref(fig:FigureIntroPriors)).
These include two state endemics, *P. condensata* Rollins and *P. dornii*
Lichvar, with rangesnextending through the Overthrust Belt mountains of central
...
central to eastern Wyoming (Figure \@ref(fig:FigureIntroPriors)).
...
ranging from 1500 – 2700 m [@Lesica2012; @OKane2010]. *P. eburniflora* is a
...
```{r FigureIntroPriors, fig.cap=caption$FigureIntroPriors}

caption$FigureIntroPriors <-
  paste("Distribution of *Physaria* sensu stricto herbarium specimens",
        "by most recent annotations.")

knitr::include_graphics(
  path = paste0("Figs/FigIntroPriors.", knit_variables$graphics)
)

```

```{r subsectionCarbonVitulifera}

knitr_section(
  knitr_title = "Status of *Physaria vitulifera* in Wyoming",
...
reported the species with a distribution East of the Front Range, on gravelly
slopes of the outer canyons in the foothills. @Ackerfield2015 also reported the
species as East of the Rocky Mountains, from the South in Fremont County to the
North in Larimer County. O'Kane's [-@OKane2010] treatment of *Physaria* follows
the latter circumscriptions, reporting *P. vitulifera* from only Colorado.
More recently, Heidel and O'Kane [-@HeidelOKane2012] identified a need for
taxonomic work based on *Physaria* collections with annotations as
*P. vitulifera*, many of which were re-annotated by Jennings (2004) as
*P. acutifolia* (indicated by “*”, Table \@ref(tab:TableIntroCarbon)).

```{r TableIntroCarbon}

# Table: Carbon county putative *P. vitulifera* annotations
caption$TableIntroCarbon <-
  ifelse(knit_variables$pandoc == "latex",
         paste0("Putative Carbon County, Wyoming ",
...
introduction_variables$carbon_specimens <- herbarium_specimens %>%
  ...
  dplyr::mutate(
    ...
    Collector = purrr::map_chr(.x = Collector, function(collector) {
      gsub("[A-Z]\\. ?", "", collector) %>% gsub("&", "and", x = .)
      }),

    # Add asterisk to specimens reviewed by Jennings (2004).
    Annotation = purrr::map2_chr(.x = prior_id, .y = Collection_Number,
      function(id, collection) {
        ifelse(test = collection %in%
                 c("411", "3695", "63", "54", "19075", "19078", "11363",
                   "16699", "19073", "16713", "19510", "6", "16475"),
               yes = paste0("*", id, "*\\*"), no = paste0("*", id, "*"))
        }),

    # Remove brackets from herbarium column of separate records.
    Herbarium = purrr::map_chr(.x = Herbarium, function(herbarium) {
      gsub("\\[|\\]", "", herbarium) %>% gsub(" +", " ", x = .)
      })
    ) %>%
  ...
  dplyr::rename(`Collection Number` = .data$Collection_Number,
                `Most Recent Annotation` = .data$Annotation) %>%
  dplyr::select(., `Most Recent Annotation`, .data$Collector,
                `Collection Number`, .data$Herbarium)

# Parse asterisks for LaTeX italicization.
introduction_variables$carbon_kable <-
  introduction_variables$carbon_specimens %>%
  dplyr::mutate(.data = .,
    "Most Recent Annotation" = purrr::map_chr(.data$`Most Recent Annotation`,
       function(x) {
  ...
  knitr::kable(x = ., format = knit_variables$kable_format,
             escape = FALSE, row.names = FALSE,
             caption = caption$TableIntroCarbon)
...
if (knit_variables$pandoc == "html" | knit_variables$pandoc == "latex") {
  ...
  introduction_variables$carbon_kable   # Simple Pandoc tibble
}

```

\paragraph{} Dorn's [-@Dorn2001] Wyoming Flora key to *Physaria* did report the
species from southcentral Wyoming, distinguished by fruits with a somewhat
rectangular outline and obtuse or truncate basal apices. It was noted the replum
often attached nearly the full length of the fruit resulting in highly reduced
apices, and that basal leaf petioles tended to be toothed. This entity was
described as occurruing on rocky slopes in or nearby mountains [@Dorn2001].
This treatment preceded a review of putative *P. vitulifera* specimens in the
state [@Jennings2004]. As previously described, the taxonomic status of
*P. vitulifera* in Wyoming is unclear
[@Rollins1993; @Dorn2001; @Jennings2004; @OKane2010; @HeidelOKane2012].
A number of specimens from Carbon county in Wyoming have previously been
annotated as *P. vitulifera* ("*Physaria vitulifera* - Carbon")
or fit the phenotype described by Dorn [-@Dorn2001]
("*Physaria acutifolia - vitulifera*-like")
appear geographically separated from the Colorado Front range *P. vitulifera*
(Figure \@ref(fig:FigureIntroCarbon)).

```{r FigureIntroCarbon, fig.cap=caption$FigureIntroCarbon}

caption$FigureIntroCarbon <-
  "Distribution of reported Carbon County, WY *Physaria vitulifera*."

knitr::include_graphics(
  path = paste0("Figs/FigIntroCarbonWyo.", knit_variables$graphics)
)
...
ascending pedicels ("Physaria"; Figure \@ref(fig:FigureIntroCarbon)).
...
```{r sectionMorphologicalTraits}

knitr_section(
  knitr_title = "Morphology and Phylogeny",
  knitr_type = "section", chunk_type = knit_variables$pandoc
)
...
Despite this marked increased in availability of sequence data, non-phylogenetic
...
To asses the status of *Physaria* species, observations of suspected
...
diagnosible monophyly from morphological characters despite the lack of
...
knitr_section(
  knitr_title = "Mechanisms of Speciation",
  knitr_type = "section", chunk_type = knit_variables$pandoc
)
...
```{r subsectionTopography}

knitr_section(
  knitr_title = "The Role of Topography in Speciation",
  knitr_type = "subsection", chunk_type = knit_variables$pandoc
)
